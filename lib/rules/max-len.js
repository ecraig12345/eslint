...
module.exports = {
    ...
    create(context) {
        ...
        function computeLineLength(line, tabWidth) {
            ...
            // Array.from ensures multi-byte unicode characters are counted only once
        ...
        function checkProgramForMaxLength(node) {
            ...
            // only get the strings, template literals, and regexp literals if ignoring them
            const strings = ignoreStrings ? getAllStrings() : [];
            const stringsByLine = strings.reduce(groupByLineNumber, {});

            const templateLiterals = ignoreTemplateLiterals ? getAllTemplateLiterals() : [];
            const templateLiteralsByLine = templateLiterals.reduce(groupByLineNumber, {});

            const regExpLiterals = ignoreRegExpLiterals ? getAllRegExpLiterals() : [];
            ...
            lines.forEach((line, i) => {
                 ...
                 * comments to check (or if there's no special handling of comments).
                 */
                ...
                const effectiveMaxLength = commentLengthApplies ? maxCommentLength : maxLength;

                // Check the line length first to avoid expensive regex checks if not needed
                if (lineLength < effectiveMaxLength ||
                    (lineIsComment && ignoreComments) ||
